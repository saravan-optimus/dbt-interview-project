...
dbt build       # builds all  models + runs  tests
```

**Expected result: `PASS=133, ERROR=0`**
...
↓
Marts Layer (10 models)       — star schema
↓
Reporting Layer (1 view)      — aggregated pipeline metrics, for Tool Consumption
↓
...
| Intermediate | 3 | int_opportunities_enriched, int_accounts_enriched, int_leads_enriched |
| Dimensions | 6 | dim_accounts, dim_users, dim_contacts, dim_campaigns, dim_products, dim_date |
| Facts | 4  | fct_opportunities (incremental), fct_cases, fct_leads, fct_opportunity_line_items (M:M bridge)|
| Snapshot | 1 | snap_accounts (SCD Type 2) |
| Reporting | 1 | fct_opportunities_summary (aggregated view by rep + fiscal period) |
...
- **1** composite key tests (dbt_utils.unique_combination_of_columns)
...
| Composite key tests | stg_case_history_2 |
...
│   │   └── _models.yml
│   ├── marts/            # 6 dims + 4 facts — BI ready star schema
│   │   └── _models.yml   # FK tests, grain docs, meta tags
│   ├── reporting/        # 1 view — aggregated pipeline metrics
│   │   └── _models.yml
...
PASS=133, WARN=0, ERROR=0, SKIP=0, NO-OP=2
